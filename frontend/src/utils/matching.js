/**
 * Driver name matching helpers (accent-insensitive, fuzzy).
 */
...
export function matchDriverName(name, drivers, options = {}) {
  ...
  if (!normalized) {
    return { best: null, candidates: [], normalized };
  }
  ...
  const autoMatch = best && (!second || best.score - second.score >= minScoreGap) && best.score >= autoMatchScore;
